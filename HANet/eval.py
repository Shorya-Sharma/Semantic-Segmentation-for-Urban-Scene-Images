...
def sliding_window_cropping(data, pos_h, scale=1.0):
    """
    ...
    for img_ctr in range(len(data)):
        ...
        for row in range(tile_rows):
            for col in range(tile_cols):
                ...
                if type(batch_pos) == type(None):
                    # print("0", pos_h.shape)
                    batch_pos = pos_h[:,y1_t:y2_t,:]
                    # print("1", batch_pos.shape)
                else:
                    batch_pos = torch.cat((batch_pos, pos_h[:,y1_t:y2_t,:]), dim=0)
                    # print("2", batch_pos.shape)
                crop_ctr += 1

    # print("mapping", mapping)
    return (mapping, sliding_window_cropping, batch_pos)
...
def pooled_eval(model, image, scale, pos):
    """
...
def inference_pool(model, img, scales, pos):
    """
    ...
    for flip in range(flip_range):
        ...
        for i in range(len(scales)):
            if type(y_tmp) == type(None):
                y_tmp = pooled_eval(model, img[flip][i], scales[i], pos=pos)
            else:
                y_tmp = pooled_eval(model, img[flip][i], scales[i], pos=pos)
...
def inference_sliding(model, img, scales, pos): ## TO DO pos also should be cropped.
    """
    ...
    for scale in scales:
        ...
        pos_h, pos_w = pos
        mapping, input_crops, batch_pos_h = sliding_window_cropping(image_list, pos_h, scale=scale)
        ...
        with torch.no_grad():
            ...
            for i in range(batch_num):
                ...
                output_crop = model(input_crop, pos=(batch_pos_h[i].unsqueeze(0), pos_w))
...
def setup_loader():
    ...
    if args.dataset == 'cityscapes':
        ...
        if args.pos_rfactor > 0:
            test_set = args.dataset_cls.CityScapesWithPos(args.mode, args.split, 0,
